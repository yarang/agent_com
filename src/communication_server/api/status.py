...
@router.get("/agents")
async def list_agents_for_management() -> dict:
    """
    List all registered agents for management.

    Returns agents with their IDs, nicknames, display IDs, and status.
    Used by the dashboard's agent management table.
    """
    registry = get_agent_registry()
    agents = await registry.get_all_agents()
...
async def delete_agent_for_management(agent_id: str) -> dict:
    """
